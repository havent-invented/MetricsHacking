...
save_filename = "vimeo11k_VSFA_200MSE_enhance_cheng2020_attn_quality2"
...
class Custom_enh_Loss(nn.Module):
    ...
    def forward(self, X_out, Y):
        ...
        self.loss["loss"] = self.loss["VSFA"] + 200*self.loss["mse_loss"] #+ loss["DISTS"] +  loss['MDTVSFA'] #+ loss["bpp_loss"] + lmbda / 2 * loss["mse_loss"] * 255 ** 2# * loss["mse"] + loss["bpp_loss"]
